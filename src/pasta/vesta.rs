...
lazy_static! {
    ...
    pub static ref MAP_VESTA: SimplifiedSWUWithDegree3Isogeny<Base, IsoAffine, Affine> = {
...
fn test_map_to_curve_vesta() {
    ...
    assert!(MAP_VESTA.theta.square() * Base::ROOT_OF_UNITY == MAP_VESTA.z);

    //let p = MAP_VESTA.map_to_curve(&Base::zero());

    let p = MAP_VESTA.map_to_curve(&Base::one());
    //println!("{:?}", p);
}
